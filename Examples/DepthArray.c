// This example mostly exists to verify that depth texture arrays work.
...
static SDL_GPUGraphicsPipeline* DepthSamplePipeline;
...
static int Init(Context* context)
{
	...
	// Creates the Shaders & Pipelines
	{
		...
		{
			SDL_Log("Failed to create Depth pipeline!");
			return -1;
		}
	...
	NearestSampler = SDL_CreateGPUSampler(context->Device, &(SDL_GPUSamplerCreateInfo){
...
static int Draw(Context* context)
{
	...
	if (swapchainTexture != NULL)
	{
		...
		SDL_BindGPUGraphicsPipeline(renderPass, DepthSamplePipeline);
		SDL_BindGPUFragmentSamplers(renderPass, 0, (SDL_GPUTextureSamplerBinding[]){
			{ .texture = SceneDepthTexture, .sampler = NearestSampler },
		}, 1);
...
Example DepthArray_Example = { "DepthArray", Init, Update, Draw, Quit };
