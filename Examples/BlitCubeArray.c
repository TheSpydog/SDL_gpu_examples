...
static int Init(Context* context)
{
	...
	SDL_GPUShader* fragmentShader = LoadShader(context->Device, "SkyboxArray.frag", 1, 1, 0, 0);
	...
	const char* imageNames[] = {
		...
		"cube5.bmp", "cube4.bmp", "cube3.bmp",
		"cube2.bmp", "cube1.bmp", "cube0.bmp"
	};
	...
	// This serves no real purpose other than demonstrating cube array->cube array blits are possible!
	for (int i = 0; i < 12; i += 1) {
		SDL_BlitGPUTexture(
			cmdbuf,
			&(SDL_GPUBlitInfo){
	...
	SDL_Log("Press Up to change the cube array index!");
...
static int Draw(Context* context)
{
	...
	if (swapchainTexture != NULL)
	{
		...
		SDL_PushGPUFragmentUniformData(cmdbuf, 0, &ArrayIndex, sizeof(Uint32));
...
Example BlitCubeArray_Example = { "BlitCubeArray", Init, Update, Draw, Quit };
